...
// --- ШАГ 2: ФУНКЦИЯ ДЛЯ СОЗДАНИЯ HTML-ОТЧЕТА ---
// Эта функция берет финальные данные и генерирует из них красивый HTML,
// используя CSS и структуру из вашего примера.
function createPdfHtml(data) {
    ...
    <head>
        ...
        <style>
            /* ВЕСЬ CSS ИЗ ВАШЕГО ПРИМЕРА CSS.TXT ВСТАВЛЕН СЮДА */
        ...
        <div class="swx-report">
            <div class="swx-header">
                <div class="swx-header-title">
                    ...
                    <span class="swx-by">by <img src="https://i.imgur.com/your-logo.png" alt="Surwix Logo" class="swx-logo" /></span>
                </div>
                <div class="swx-meta">
            ...
            <div class="swx-section">
                ...
                <table class="swx-table">
                    <thead><tr><th>Threat</th><th>Level</th><th>Advice</th><th>Probability</th></tr></thead>
                    <tbody>${generateRiskRows()}</tbody>
                </table>
            </div>
            <div class="swx-section">
                <h2>Recent Disasters <span class="src-badge">OpenFEMA</span></h2>
                <ul>${data.recent_disasters ? data.recent_disasters.map(d => `<li>${d}</li>`).join('') : '<li>No major disasters declared recently.</li>'}</ul>
                <small class="source">Source: OpenFEMA.gov</small>
            </div>
            <div class="section checklist">
                <h2><i class="fa fa-clipboard-check"></i> Evacuation Checklist</h2>
                <ul class="checklist-list">
                    ${data.checklist ? data.checklist.map(item => `<li><i class="fa fa-check-circle"></i> ${item}</li>`).join('') : ''}
                </ul>
            </div>
            <div class="swx-footer">
                <small>Data for risk analysis provided by FEMA, NOAA, and AI analysis.<br/>This report is generated by Surwix AI. Not a legal notice. Always follow instructions from local authorities.</small>
            </div>
        </div>
    </body>
    </html>
...
export default async function handler(request, response) {
    ...
    try {
        ...
        let recentDisasters = [];
        try {
            // Извлекаем штат из адреса для более точного запроса
            ...
            console.error("Could not fetch from OpenFEMA, proceeding without it.", e);
        }
        console.log('OpenFEMA data received.');

        // --- ШАГ 2: ФОРМИРУЕМ МОЩНЫЙ ПРОМПТ ДЛЯ OPENAI ---
        console.log('[2/4] Generating AI data...');
        const openai = new OpenAI({ apiKey: process.env.OPENAI_API_KEY });
        const prompt = `
            Act as a U.S. emergency preparedness expert for the address: "${address}".
            Recent FEMA-declared disasters in this state include: ${recentDisasters.join(', ') || 'None'}.
            ...
            2.  "hazard_matrix": An array of exactly 3 objects. Each object must have "threat" (string), "level" (string), "level_color" (string), "advice" (string), and "probability" (a short string with mock data, e.g., "1 in 100 year zone").
            3.  "checklist": An array of 6 short, actionable strings for an evacuation checklist.
            
            Base your analysis on the provided address and recent disaster data to generate plausible, realistic mock data for all fields.
        `;
        ...
        reportData.recent_disasters = recentDisasters; // Добавляем данные FEMA в отчет
